"""
final_fast_w_comparison.py

Faster final Streamlit population projection app with optional ISTAT/UN comparison.

This version keeps the interface and downloadable outputs of
`final_streamlit_fast.py`, keeps the NumPy projection engine, and adds:

    - optional comparison switch for clean vs benchmark figures;
    - total-population comparison with ISTAT and UN medians plus intervals;
    - optional ISTAT age-sex overlay in the population pyramid.
...
The demographic order is unchanged:
    1. multiply each age/sex/citizenship population by survival probabilities;
    2. subtract emigration and add immigration;
    3. compute births from post-migration female population and ASFR;
    4. age everyone forward by one year;
    5. insert newborns at age 0.
...
LOCAL_FALLBACK_SOURCE_APP_PATH = APP_DIR / "streamlit_projection_app copy.py"


@st.cache_resource(show_spinner=False)
def load_source_app():
    """Load helper functions from GitHub's final_streamlit.py, with local fallback."""

    source_path = SOURCE_APP_PATH if SOURCE_APP_PATH.exists() else LOCAL_FALLBACK_SOURCE_APP_PATH
    module_name = "final_streamlit_helpers"
    spec = importlib.util.spec_from_file_location(module_name, source_path)
    if spec is None or spec.loader is None:
        raise ImportError(f"Could not load helper app from {source_path}")
...
def load_official_population_comparison_tables(data_dir_str: str) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Load ISTAT and UN comparison files for total population and ISTAT pyramid.

    Expected files in the repository/input folder:
        - istat.csv with year, sex, age, low95, median, high95, source
        - un.csv with year, age, low90, median, high90, source

    ISTAT is already in persons.  UN population-pyramid style data are commonly
    stored in thousands, so this loader automatically scales UN to persons when
    the yearly totals are clearly too small.
    """

    data_dir = Path(data_dir_str)
    total_frames = []
    istat_age_sex = pd.DataFrame(columns=["source", "year", "sex", "age", "median", "lower", "upper", "interval"])

    istat_path = data_dir / "istat.csv"
    if istat_path.exists():
        istat = pd.read_csv(istat_path)
        istat = istat.rename(columns={c: str(c).strip().lower() for c in istat.columns})
        required = {"year", "sex", "age", "median", "low95", "high95"}
        if required.issubset(istat.columns):
            istat["year"] = pd.to_numeric(istat["year"], errors="coerce")
            istat["age"] = pd.to_numeric(istat["age"], errors="coerce")
            for col in ["median", "low95", "high95"]:
                istat[col] = pd.to_numeric(istat[col], errors="coerce")
            istat = istat.dropna(subset=["year", "age", "median", "low95", "high95"]).copy()
            istat["year"] = istat["year"].astype(int)
            istat["age"] = istat["age"].clip(0, 100).astype(int)
            istat["source"] = "ISTAT"
            istat["sex"] = istat["sex"].astype(str).str.lower().map(
                {
                    "maschi": "male",
                    "maschio": "male",
                    "male": "male",
            ...
            istat = istat[istat["sex"].isin(["male", "female"])].copy()
            istat_age_sex = istat.rename(columns={"low95": "lower", "high95": "upper"})[
                ["source", "year", "sex", "age", "median", "lower", "upper"]
            ].copy()
            istat_age_sex["interval"] = "95%"
            total = istat.groupby("year", as_index=False)[["median", "low95", "high95"]].sum()
            total = total.rename(columns={"low95": "lower", "high95": "upper"})
            total["source"] = "ISTAT"
            total["interval"] = "95%"
            total_frames.append(total[["source", "year", "median", "lower", "upper", "interval"]])

    un_path = data_dir / "un.csv"
    if un_path.exists():
        un = pd.read_csv(un_path)
        un = un.rename(columns={c: str(c).strip().lower() for c in un.columns})
        required = {"year", "age", "median", "low90", "high90"}
        if required.issubset(un.columns):
            un["year"] = pd.to_numeric(un["year"], errors="coerce")
            un["age"] = pd.to_numeric(un["age"], errors="coerce")
            for col in ["median", "low90", "high90"]:
                un[col] = pd.to_numeric(un[col], errors="coerce")
            un = un.dropna(subset=["year", "age", "median", "low90", "high90"]).copy()
            un["year"] = un["year"].astype(int)
            total = un.groupby("year", as_index=False)[["median", "low90", "high90"]].sum()
            total = total.rename(columns={"low90": "lower", "high90": "upper"})
            if total["median"].max() < 1_000_000:
                total[["median", "lower", "upper"]] = total[["median", "lower", "upper"]] * 1000.0
            total["source"] = "UN"
            total["interval"] = "90%"
            total_frames.append(total[["source", "year", "median", "lower", "upper", "interval"]])

    if total_frames:
        total_comparison = pd.concat(total_frames, ignore_index=True).sort_values(["source", "year"])
    else:
        total_comparison = pd.DataFrame(columns=["source", "year", "median", "lower", "upper", "interval"])
    return total_comparison, istat_age_sex


def plot_total_population_with_official_comparison(
    summary: pd.DataFrame,
    scenario_names: list[str],
    policy_start_year: int,
    policy_end_year: int,
    official_total: pd.DataFrame,
    show_comparisons: bool,
) -> plt.Figure:
    """Plot projected total population plus optional ISTAT/UN intervals."""
    ...
    for _, g in summary[summary["scenario"].isin(scenario_names)].groupby("scenario"):
        ...
        ax.plot(years, g["p50"].to_numpy(float), linewidth=2.4, label=g["scenario_label"].iloc[0])
        ax.fill_between(years, g["p10"].to_numpy(float), g["p90"].to_numpy(float), alpha=0.14)

    if show_comparisons and not official_total.empty:
        colors = {"ISTAT": "#D62728", "UN": "#2CA02C"}
        for source, g in official_total.groupby("source"):
            g = g.sort_values("year")
            years = g["year"].to_numpy(float)
            color = colors.get(str(source).upper(), None)
            interval = str(g["interval"].iloc[0]) if "interval" in g.columns and len(g) else "CI"
            ax.plot(
                years,
                g["median"].to_numpy(float),
                linestyle="--",
                linewidth=2.0,
                color=color,
            ...
            ax.fill_between(
                years,
                g["lower"].to_numpy(float),
                g["upper"].to_numpy(float),
                color=color,
                alpha=0.10,
                label=f"{source} {interval} interval",
            )

    ax.set_title("Total population projection")
    ax.set_xlabel("Year")
...
def plot_pyramid_with_istat(
    pop_mean: pd.DataFrame,
    istat_age_sex: pd.DataFrame,
    scenario_name: str,
    year: int,
    show_comparisons: bool,
) -> plt.Figure:
    """Population pyramid with optional ISTAT age-sex profile overlay."""

    g = pop_mean[(pop_mean["scenario"].eq(scenario_name)) & (pop_mean["year"].eq(int(year)))].copy()
    fig, ax = plt.subplots(figsize=(8, 7))
    if g.empty:
        ax.set_title("Population pyramid")
        ax.text(0.5, 0.5, "No projection data for selected scenario/year", ha="center", va="center", transform=ax.transAxes)
        fig.tight_layout()
        return fig

    pivot = g.pivot_table(index="age", columns="sex", values="population", aggfunc="sum").reindex(range(101)).fillna(0.0)
    male = -pivot.get("male", pd.Series(0.0, index=range(101))).to_numpy(float)
    female = pivot.get("female", pd.Series(0.0, index=range(101))).to_numpy(float)
    ages = np.arange(101)
    ax.barh(ages, male, color="#4C78A8", alpha=0.72, label="Our projection male")
    ax.barh(ages, female, color="#F58518", alpha=0.72, label="Our projection female")

    if show_comparisons and not istat_age_sex.empty:
        istat = istat_age_sex[istat_age_sex["year"].eq(int(year))].copy()
        if not istat.empty:
            ip = istat.pivot_table(index="age", columns="sex", values="median", aggfunc="sum").reindex(range(101)).fillna(0.0)
            imale = -ip.get("male", pd.Series(0.0, index=range(101))).to_numpy(float)
            ifemale = ip.get("female", pd.Series(0.0, index=range(101))).to_numpy(float)
            ax.plot(imale, ages, color="#111111", linewidth=1.8, label="ISTAT male")
            ax.plot(ifemale, ages, color="#111111", linewidth=1.8, linestyle="--", label="ISTAT female")

    max_abs = max(abs(male).max(), abs(female).max(), 1.0)
...
def summarize_population_by_age_sex_origin(
    ...
    """Summarize population by age, sex and origin with vectorized quantiles."""

    origin_labels = {"italiani": "Italy", "stranieri": "Foreign countries"}
    mean = pop_age_sex_origin.mean(axis=1)
    qs = np.quantile(pop_age_sex_origin, [0.05, 0.10, 0.50, 0.90, 0.95], axis=1)

    rows = []
    for scen_idx, y_idx, c_idx, s_idx, age_idx in product(
        range(len(scenarios)),
        range(len(years)),
        range(len(engine.CITIZENSHIPS)),
        range(len(engine.SEXES)),
        range(len(engine.AGES)),
    ):
        citizenship = engine.CITIZENSHIPS[c_idx]
        rows.append(
            {
                "scenario": scenarios[scen_idx].name,
                "scenario_label": scenarios[scen_idx].label,
                "year": int(years[y_idx]),
                "origin": origin_labels.get(citizenship, citizenship),
                "citizenship": citizenship,
                "sex": engine.SEXES[s_idx],
                "age": int(engine.AGES[age_idx]),
                "population_mean": float(mean[scen_idx, y_idx, c_idx, s_idx, age_idx]),
                "population_p05": float(qs[0, scen_idx, y_idx, c_idx, s_idx, age_idx]),
                "population_p10": float(qs[1, scen_idx, y_idx, c_idx, s_idx, age_idx]),
                "population_p50": float(qs[2, scen_idx, y_idx, c_idx, s_idx, age_idx]),
                "population_p90": float(qs[3, scen_idx, y_idx, c_idx, s_idx, age_idx]),
                "population_p95": float(qs[4, scen_idx, y_idx, c_idx, s_idx, age_idx]),
            }
        )
...
def save_state_fast(
    ...
    """Store one simulated population state without converting to DataFrame."""
    ...
    if pop_age_sex_origin is not None:
        pop_age_sex_origin[scen_idx, sim_idx, year_idx, :, :, :] = state


def project_one_year_fast(engine, state: np.ndarray, px: np.ndarray, asfr: np.ndarray, immigration: np.ndarray, emigration: np.ndarray):
    """Fast NumPy version of survive -> migrate -> births -> age forward."""
    ...
    female = engine.SEX_TO_INDEX["female"]
    births_by_citizenship = (post_migration[:, female, :] * asfr).sum(axis=1)
    total_births = float(births_by_citizenship.sum())

    next_state = np.zeros_like(state)
    next_state[:, :, 1:] = post_migration[:, :, :-1]
    next_state[:, :, 100] += post_migration[:, :, 100]
    next_state[:, engine.SEX_TO_INDEX["male"], 0] = births_by_citizenship * engine.SEX_RATIO_AT_BIRTH_MALE
    next_state[:, engine.SEX_TO_INDEX["female"], 0] = births_by_citizenship * (1.0 - engine.SEX_RATIO_AT_BIRTH_MALE)
...
def run_projection_classic(
    ...
    foreign_tfr_increase_pct: float,
    tfr_increase_mode: str,
    include_age_origin_summary: bool = False,
) -> dict:
    ...
    # Main loop.  This is the same all-at-once cohort-component projection as in
    # the readable version, but it stays in NumPy arrays.  Avoiding thousands of
    # small pandas DataFrame copies is the main Streamlit speed improvement.
    for scen_idx, scenario in enumerate(scenarios):
        for sim_idx in range(n_sim):
            state = initial_state.copy()
            save_state_fast(engine, state, scen_idx, sim_idx, 0, total_pop, oadr, pop_age_sex, pop_age_sex_origin)
            ...
            for y_idx, year in enumerate(transition_years):
                ...
                total_tfr[scen_idx, sim_idx, y_idx] = engine.compute_total_tfr_from_state(state, asfr)
                state, total_births, applied_emigration = project_one_year_fast(engine, state, px, asfr, immigration, emigration)
                ...
                save_state_fast(engine, state, scen_idx, sim_idx, y_idx + 1, total_pop, oadr, pop_age_sex, pop_age_sex_origin)
    ...
    if include_age_origin_summary:
        ...
        population_age_sex_origin_summary = pd.DataFrame(
            columns=[
                "scenario",
                "scenario_label",
                "year",
    ...
    return {
        ...
        "population_age_sex_origin_summary": population_age_sex_origin_summary,
        ...
        "projection_style": "fast_numpy_age_survive_migrate_birth_age_forward",
    }


def deterministic_population_path_classic(engine, components: dict, scenario) -> pd.DataFrame:
    """Central deterministic path used by reverse calculators, in fast arrays."""
    ...
    rows = [
        {
            ...
            "population": float(state.sum()),
            "oadr": float(100.0 * state[:, :, 65:].sum() / working) if working > 0 else np.nan,
            "immigration": 0.0,
            "emigration": 0.0,
            "net_migration": 0.0,
        }
    ...
    for y_idx, year in enumerate(components["transition_years"]):
        ...
        state, _, applied_emigration = project_one_year_fast(engine, state, px, asfr, immigration, emigration)
        working = state[:, :, 15:65].sum()
        rows.append(
            {
                "year": int(year + 1),
...
def run_classic_cached(
    ...
    foreign_tfr_increase_pct: float,
    tfr_increase_mode: str,
    include_age_origin_summary: bool,
    ...
    """Cached Streamlit wrapper around the fast NumPy projection."""
    ...
    return run_projection_classic(
        ...
        foreign_tfr_increase_pct=float(foreign_tfr_increase_pct),
        tfr_increase_mode=tfr_increase_mode,
        include_age_origin_summary=bool(include_age_origin_summary),
    )
...
def make_model_parameter_table(
    ...
    rows = [
        ("app", "file", "final_streamlit_fast.py"),
        ...
        ("comparison", "optional_comparison_files", "istat.csv, un.csv"),
...
def dataframe_to_csv_bytes(table: pd.DataFrame) -> bytes:
    """Cache CSV serialization so Streamlit reruns do not redo it unnecessarily."""
...
def main() -> None:
    ...
    st.set_page_config(page_title="Italy projection - fast final cohort component", layout="wide")
    st.title("Population Projection Simulator")
    st.caption(
        "Fast final version: raw-input component forecasts, NumPy age-survive-migrate-birth-age-forward projection logic, "
    ...
    st.sidebar.header("Comparison Display")
    show_comparisons = st.sidebar.checkbox(
        "Show comparisons with official sources",
        value=True,
        help="Turn off for clean figures showing only this model's projections.",
    )
    ...
    n_sim = st.sidebar.slider("Simulation paths", min_value=10, max_value=200, value=30, step=10)
    ...
    include_age_origin_summary = st.sidebar.checkbox(
        "Prepare detailed age-sex-origin CSV",
        value=False,
        help="Leave off for faster interaction. Turn on when you need the full downloadable age-sex-origin projection table.",
    )
    ...
    try:
        results = run_classic_cached(
            ...
            tfr_increase_mode,
            bool(include_age_origin_summary),
    ...
    if show_comparisons:
        ...
        official_total_comparison, istat_age_sex_comparison = load_official_population_comparison_tables(str(data_dir))
    else:
        migration_benchmarks = pd.DataFrame()
        tfr_benchmarks = pd.DataFrame()
        e0_benchmarks = pd.DataFrame()
        official_total_comparison = pd.DataFrame(columns=["source", "year", "median", "lower", "upper", "interval"])
        istat_age_sex_comparison = pd.DataFrame(columns=["source", "year", "sex", "age", "median", "lower", "upper", "interval"])
    ...
    st.pyplot(
        plot_total_population_with_official_comparison(
            total,
            list(selected_names),
            int(policy_start_year),
            int(policy_end_year),
            official_total_comparison,
            bool(show_comparisons),
        ),
        clear_figure=True,
    )
    if show_comparisons and official_total_comparison.empty:
        st.caption("No ISTAT/UN total-population comparison files found. Add istat.csv and/or un.csv to the input folder.")
    ...
    st.pyplot(
        plot_pyramid_with_istat(
            pop_mean,
            istat_age_sex_comparison,
            pyramid_scenario,
            int(pyramid_year),
            bool(show_comparisons),
        ),
    ...
    if show_comparisons and istat_age_sex_comparison.empty:
        st.caption("ISTAT age-sex profile was not available for pyramid comparison.")
    ...
    with population_game:
        ...
        if run_reverse:
            ...
            st.download_button(
                ...
                dataframe_to_csv_bytes(reverse),
    ...
    with oadr_game:
        ...
        if run_oadr_reverse:
            ...
            st.download_button(
                ...
                dataframe_to_csv_bytes(reverse_oadr),
                file_name="reverse_required_immigration_for_oadr.csv",
                mime="text/csv",
            )

    with st.expander("Download result tables"):
        st.caption("To keep the app fast, CSV files are prepared only when this box is selected.")
        prepare_downloads = st.checkbox("Prepare downloadable CSV files", value=False)
        if prepare_downloads:
            ...
            if show_comparisons:
                download_tables["official_population_comparison_total.csv"] = official_total_comparison
                download_tables["istat_age_sex_comparison.csv"] = istat_age_sex_comparison
            if pop_age_origin.empty:
                st.warning(
                    "The detailed age-sex-origin table was not prepared. Turn on "
                    "'Prepare detailed age-sex-origin CSV' in the sidebar and rerun if you need that file."
                )
            else:
                download_tables["projection_age_sex_origin_summary.csv"] = pop_age_origin
            st.download_button(
